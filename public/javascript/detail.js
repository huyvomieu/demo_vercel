...
document
  .getElementById("acceptMessageButton")
...
/////// fix lỗi hiển thị buy ticket không hiện khi chưa chọn nagyf và ghếghế
// document.addEventListener("DOMContentLoaded", function () {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const submitSeatsButton = document.getElementById("submit-seats"); // Nút chọn ghế

  // Mặc định vô hiệu hóa nút Buy Ticket nếu tồn tại
  if (buyTicketButton) {
    buyTicketButton.disabled = true;
  }

  let seatSelected = false; // Trạng thái chọn ghế
  let timeSelected = false; // Trạng thái chọn thời gian

  // Kiểm tra ghế đã được chọn
  if (submitSeatsButton) {
    submitSeatsButton.addEventListener("click", function () {
      seatSelected = document.querySelectorAll(".seat.selected").length > 0; // Có ghế nào được chọn không
      updateBuyTicketStatus(); // Cập nhật trạng thái nút Buy Ticket
    });
  }

  // Kiểm tra thời gian chiếu đã được chọn
  if (submitTimeButton) {
    submitTimeButton.addEventListener("click", function () {
      const dateInput = document.getElementById("dateInput")?.value; // Lấy giá trị ngày chiếu
      const partOfDay = document.getElementById("timeofTicket")?.value; // Lấy giá trị thời gian chiếu

      if (dateInput && partOfDay) {
        timeSelected = true; // Đã chọn thời gian
        updateDateTime(dateInput, partOfDay); // Cập nhật thông tin ngày giờ
      } else {
        timeSelected = false; // Chưa chọn đủ thời gian
        alert("Vui lòng chọn đầy đủ ngày và thời gian chiếu.");
      }
      updateBuyTicketStatus(); // Cập nhật trạng thái nút Buy Ticket
    });
  }

  // Hàm cập nhật trạng thái nút Buy Ticket
  function updateBuyTicketStatus() {
    if (seatSelected && timeSelected) {
      enableBuyTicketButton(); // Kích hoạt nút khi đủ điều kiện
    } else {
      disableBuyTicketButton(); // Giữ nút mờ đi nếu chưa chọn đủ
    }
  }

  // Hàm kích hoạt nút Buy Ticket
  function enableBuyTicketButton() {
    if (buyTicketButton) {
      buyTicketButton.disabled = false;
    }
  }

  // Hàm vô hiệu hóa nút Buy Ticket
  function disableBuyTicketButton() {
    if (buyTicketButton) {
      buyTicketButton.disabled = true;
    }
  }

  // Hàm cập nhật thông tin ngày và giờ chiếu vào giao diện
  function updateDateTime(date, time) {
    const dateTimeElement = document.getElementById("datetimeticket");
    if (dateTimeElement) {
